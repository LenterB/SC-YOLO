...
def bbox_iou(box1, box2, xywh=True, ratio=1, GIoU=False, DIoU=False, CIoU=False, SIoU=False,
             EIoU=False, WIoU=False, MPDIoU=False, LMPDIoU=False, Inner=False, Focal=False, OIoU=False,alpha=1, gamma=0.5,
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU or MPDIoU or LMPDIoU or OIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU or WIoU or MPDIoU or LMPDIoU:  # Distance or Complete IoU
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                cw2 = torch.pow(cw ** 2 + eps, alpha)
                ch2 = torch.pow(ch ** 2 + eps, alpha)
                extern_loss=rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2
                if Focal:
                    return iou - torch.pow(0.3333 * (distance_cost + shape_cost+extern_loss) + eps, alpha), torch.pow(
                        inter / (union + eps), gamma)  # Focal_SIou
                else:
                    return iou - torch.pow(0.3333 * (distance_cost + shape_cost+extern_loss) + eps, alpha)  # SIou
...
class SegmentMetrics(SimpleClass):
    ...
    def keys(self):
        ...
        return [
            ...
            'metrics/precision(M)', 'metrics/recall(M)', 'metrics/mAP50(M)', 'metrics/mAP75(B)','metrics/mAP50-95(M)']
...
class PoseMetrics(SegmentMetrics):
    ...
    def keys(self):
        ...
        return [
            'metrics/precision(B)', 'metrics/recall(B)', 'metrics/mAP50(B)', 'metrics/mAP75(B)','metrics/mAP50-95(B)',
            'metrics/precision(P)', 'metrics/recall(P)', 'metrics/mAP50(P)', 'metrics/mAP75(B)','metrics/mAP50-95(P)']
