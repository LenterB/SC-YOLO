...
__all__ = ('DFL', 'HGBlock', 'HGStem', 'SPP', 'SPPF', 'C1', 'C2', 'C3', 'C2f', 'C3x', 'C3TR', 'C3Ghost',
           'GhostBottleneck', 'Bottleneck', 'BottleneckCSP', 'Proto', 'RepC3','ResNetLayer','ResNetLayer18','BasicBlock')
...
class SPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        y1 = self.m(x)
        y2 = self.m(y1)
        temp=self.cv2(torch.cat((x, y1, y2, self.m(y2)), 1))
        #print("SPPF")
        #print(temp.shape)
        return temp
...
class C3(nn.Module):
    ...
    def forward(self, x):
        ...
        temp=self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))
        #print("C3:")
        #print(temp.shape)
        return temp
...
class ResNetLayer18(nn.Module):
    ...
    def __init__(self, c1, c2, s=1, is_first=False, n=1, e=4):
        ...
        if self.is_first:
            ...
            blocks.extend([ResNetBlock18(c2, c2, 1) for _ in range(n - 1)])
            self.layer = nn.Sequential(*blocks)
...
class Silence(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
class Bottleneck_ghost(nn.Module):
    """Standard bottleneck."""
...
class C2f_ghost(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.ModuleList(Bottleneck_ghost(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
