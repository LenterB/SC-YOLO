...
__all__ = ('Conv', 'LightConv', 'DWConv', 'DWConvTranspose2d', 'ConvTranspose', 'Focus', 'GhostConv',
           'ChannelAttention', 'SpatialAttention', 'CBAM', 'Concat', 'RepConv','CoordAtt','Add')
...
class ConvTranspose(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(self.conv_transpose(x).shape)
...
class GhostConv(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.cv1(x)
        #print("GhostConv finish")
        #print(torch.cat((y, self.cv2(y)), 1).shape)
...
class Concat(nn.Module):
    ...
    def forward(self, x):
        ...
        #rint(x[0].shape,x[1].shape)
        return torch.cat(x, self.d)
    
class ConcatWithAttention(nn.Module):
    ...
    def forward(self, x):
        concatenated_features = torch.cat(x, self.d)
        #print(concatenated_features.shape)
        attended_features = self.attention(concatenated_features)
